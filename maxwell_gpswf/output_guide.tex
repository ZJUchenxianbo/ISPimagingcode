...
The Full VIE source first solves
\begin{equation}
    \mathbf{E}(\mathbf{x})
...
\end{equation}
and then evaluates the far-field integral with the computed total field. Hence
...
All rows use the same three-block phantom, 974 incident directions, 974
observation directions, target quadrature nodes, mock configurations,
...
\end{verbatim}
Experiment 5 is deliberately excluded from \texttt{--mode all} because the
formal Full VIE solve is substantially more expensive. It is run explicitly
with
\begin{verbatim}
.venv/bin/python maxwell_gpswf/main.py \
